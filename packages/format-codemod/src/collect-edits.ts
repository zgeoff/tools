...
function statementLists(node: AstNode): (readonly AstNode[])[] {
...
function nodeValues(value: unknown): AstNode[] {
...
function childNodes(node: AstNode): AstNode[] {
  ...
  for (const key of Object.keys(node)) {
    if (key !== 'loc' && key !== 'parent') {
      children.push(...nodeValues(node[key]));
    }
  }
...
function pairEdits(src: string, container: AstNode, body: readonly AstNode[]): Edit[] {
  const edits: Edit[] = [];
...
function walk(src: string, node: AstNode): Edit[] {
  ...
  for (const body of statementLists(node)) {
    edits.push(...pairEdits(src, node, body));
  }
...
export function collectEdits(src: string, ast: AstNode): Edit[] {
